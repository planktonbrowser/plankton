...
void blitFramebuffer(QOpenGLFramebufferObject *target,
                     ::std::unique_ptr<QOpenGLFramebufferObject> source) {
...
fboFromTexture(unsigned int texture_id, unsigned int texture_target,
               QSize size) {
...
::rust::isize qTouchEventPointCount(QTouchEvent const &event) {
  return static_cast<::rust::isize>(event.pointCount());
}

QEventPoint const &qTouchEventPoint(QTouchEvent &event, ::rust::isize index) {
